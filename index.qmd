...
This is a modeling tutorial for the supplement for Miller-ter Kuile et al. "If you’re rare, should I care? How imperfect detection changes relationships between biodiversity and global change drivers". *in revision*
...
Other options are available for running Bayesian models, including [Stan](https://mc-stan.org/) and [NIMBLE](https://r-nimble.org/). There is less flexibility but great usability for running multi-species occupancy models in the package [`spOccupancy`](https://doserlab.com/files/spoccupancy-web/). Given the incorporation of uncertainty in regression models, it would be challenging to run them in programs with built-in regression model functions, but they can be built in any Bayesian modeling language/program (JAGS, Stan, BUGS, NIMBLE).
