...
You will need to provide a path to the model file (which is its own R script, written in JAGS/BUGS language, so it won't actually run in R). You can find ours [here](https://github.com/an-bui/community_detection_tutorial/blob/main/tutorials/01_imperfect_detect/code/MSAM_model.R). You will see that we define the path to this model in our model running script below.

#### The data list

To run the model, we will need to provide JAGS with a data list, which you can find [here](https://github.com/an-bui/community_detection_tutorial/tree/main/tutorials/01_imperfect_detect/data/model_inputs). We have code on how to prepare the data list [here](https://github.com/an-bui/community_detection_tutorial/blob/main/tutorials/01_imperfect_detect/code/MSAM_data_simulation.R).
...
We'll run the model using the `jagsUI` wrapper package. You can find this script [here](https://github.com/an-bui/community_detection_tutorial/blob/main/tutorials/01_imperfect_detect/code/MSAM_wrapper_script.R), and the general code to run a JAGS model is provided here:
...
### Next: using N, $z$, or $\psi$ to compute indices of biodiversity
